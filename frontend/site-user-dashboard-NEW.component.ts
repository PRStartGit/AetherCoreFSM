...
import { Task, Defect, DefectStatus } from '../../../core/models/monitoring.model';
...
export class SiteUserDashboardComponent implements OnInit {
  ...
  loadTasks(): void {
    ...
    const siteId = this.currentUser?.user_sites?.[0]?.site_id;
  ...
  processTasks(tasks: Task[], siteId: number): void {
    ...
    const siteTasks = tasks.filter(t => {
      // Check if task is assigned to this site via site_ids or assigned_sites array
    ...
    siteTasks.forEach(task => {
      const closesAt = task.category?.closes_at;

      if (!closesAt) {
        console.log(`Task ${task.id} has no closes_at time, skipping`);
        return;
      }

      // Parse closes_at time (format: "HH:MM:SS" or "HH:MM")
      ...
      const taskCard: TaskCard = {
        id: task.id!,
        taskName: task.name,
        categoryName: task.category?.name || 'Unknown Category',
        closesAt: this.formatClosesAt(closesAt),
        timeInfo: '',
        status: 'open'
      };

      // Categorize based on current time vs close time
      if (currentTime > closeTimeMinutes) {
        // MISSED - category close time has passed
  ...
  loadMyDefects(): void {
    ...
    this.defectService.getAll().subscribe({
      next: (defects) => {
        // Filter open defects reported by current user
  ...
  viewTask(taskId: number): void {
    // Navigate to task details or checklist completion
